...
n_obs = 6
n_actions = 2
net_sample = CSnet(n_obs, n_actions)
...
game_params = {"MAX_TIME"     : 500, # Game will end after MAX_TIME
               ...
               "Param_Range"  : [[0,    0],     # 0) dummy variable
                                 ...
                                 [base_flap * (1-percent), base_flap * (1+percent)],  # 5) BIRD_Lift
...
def prepare_params():
    # Output: params = [seed, lift, gravity, fwd_flap, drag] * REPEATS
    params = []
...
def eval_Tasks(ind):
    ...
    count = 0
    # [Task-1: Lift, Task-2: Gravity, Task-3: Fwd-Flap, Task-4: Drag]
    for task in range(4):
...
def main(pickle_file=None):
    ...
    with open("gen%d_CS_checkpoint.pkl" % (gen), "wb") as cp_file:
...
if __name__ == "__main__":
    ...
    pop = main()
    pickle.dump(pop, open("popAsNets_CS.p","wb"))
